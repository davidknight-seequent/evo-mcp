...
def register_admin_tools(mcp):
    ...
    async def get_workspace_summary(
        ...
        if ctx:
            await ctx.info(
                "Workspace summary complete",
                extra={"total_objects": result["total_objects"], "total_files": result["total_files"]},
            )
                ...
                if include_data_blobs:
                    try:
                        ...
                        await log_handled_failure(
                            ctx,
                            logger,
                            "Failed to extract data blobs for snapshot object",
                            operation_id,
                            e,
                            ctx_level="warning",
                            workspace_id=workspace_id,
    ...
    async def workspace_copy_object(
        ...
        try:
            ...
            source_object = await source_client.download_object_by_id(
                UUID(object_id), version=version if version else None
            )
            if ctx:
            ...
            await log_operation_event(
                ctx,
                logger,
                "Workspace object copy completed",
                operation_id,
                source_workspace_id=source_workspace_id,
                target_workspace_id=target_workspace_id,
                object_id=result["id"],
            ...
            await log_handled_failure(
                ctx,
                logger,
                "Failed to copy object between workspaces",
                operation_id,
                e,
                source_workspace_id=source_workspace_id,
                target_workspace_id=target_workspace_id,
